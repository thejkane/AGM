...
data = []
header = ('Nodes', 'TasksPerNode', 'Threads', 'Cores', 'File', 'Degree', 'Sources', 'Coalescing', 'Flush', 'Algorithm', 'TMEAN','TSTDEV','TMIN','TQ1','TMEDIAN', 'TQ3','TMAX','TEPMIN','TEPQ1','TEPMEDIAN','TEPQ3','TEPMAX','TEPMEAN','TEPSTDEV')
for inputfile in glob.glob("*.o*"):
	...
	with open(inputfile) as input:
		...
		m = re.search(r'\+ aprun -b -n ([0-9]+) -N ([0-9]+) -d ([0-9]+) ../../../rmat1/bin/performance_test --poll-task 1 --threads ([0-9]+) --graph-file ((.+?)(\.[^.]*)) --degree ([0-9]+) --num-sources ([0-9]+)', line)
		if m != None:
		    tasks = int(m.group(1))
		    ...
		    cores = threads * tasks
		    currentscalenodetpl = (nodes, taskpernode, m.group(4), cores, m.group(5), m.group(8), m.group(9))

		m = re.search(r'\+ aprun -b -n ([0-9]+) -N ([0-9]+) -d ([0-9]+) ../performance_test --poll-task 1 --threads ([0-9]+) --graph-file ((.+?)(\.[^.]*)) --degree ([0-9]+) --num-sources ([0-9]+)', line)
		if m != None:
		    tasks = int(m.group(1))
		    taskpernode = int(m.group(2))
		    nodes = tasks / taskpernode
		...
		m = re.search(r'\+ aprun -b -n ([0-9]+) -N ([0-9]+) -d ([0-9]+) ../bin/(\w+) --poll-task 1 --threads ([0-9]+) --scale ([0-9]+) --degree ([0-9]+) --num-sources ([0-9]+)', line)
		if m != None:
		    tasks = int(m.group(1))
		    taskpernode = int(m.group(2))
		    nodes = tasks / taskpernode
...
#setdata = set(data, key=lambda tup: str(tup[4])+tup[10])
sorteddata = sorted(data, key=lambda tup: tup[9]+str(tup[3]))

dictdata = {}		
for tup in sorteddata:
	key=tup[9]+str(tup[3])
	if key in dictdata:
		if (float(dictdata[key][10])) > float(tup[10]):
			dictdata[key] = tup
	else:
		dictdata[key] = tup
...
with open(outputfile, 'wb') as csvfile:
    ...
    sorteddict = sorted(dictdata)
    for k,d in dictdata.items():
        print k
