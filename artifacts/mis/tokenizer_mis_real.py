...
data = []
header = ('Nodes', 'Binary', 'Threads', 'Cores', 'Processes', 'File', 'Degree', 'Sources', 'Coalescing', 'Flush', 'Algorithm', 'TMEAN','TSTDEV','TMIN','TQ1','TMEDIAN', 'TQ3','TMAX','TEPMIN','TEPQ1','TEPMEDIAN','TEPQ3','TEPMAX','TEPMEAN','TEPSTDEV')
for inputfile in glob.glob("*.o*"):
	...
	with open(inputfile) as input:
		...
		m = re.search(r'\+ aprun -b -n ([0-9]+) -N ([0-9]+) -d ([0-9]+) -r 1 ../(\w+) --poll-task 1 --threads ([0-9]+) --graph-file ((.+?)(\.[^.]*)) --degree ([0-9]+) --num-sources ([0-9]+)', line)
		if m != None:
		    tasks = int(m.group(1))
		    taskpernode = int(m.group(2))
		    cores = int(m.group(3)) + 1
		    totalcores = cores * tasks
		    nodes = tasks / taskpernode
		    currentscalenodetpl = (nodes, m.group(4), m.group(5), totalcores, tasks, m.group(6), m.group(7), m.group(8))

		m = re.search(r'\+ aprun -b -n ([0-9]+) -N ([0-9]+) -d ([0-9]+) ../(\w+) --poll-task 1 --threads ([0-9]+) --graph-file ((.+?)(\.[^.]*)) --degree ([0-9]+) --num-sources ([0-9]+)', line)
		if m != None:
		    tasks = int(m.group(1))
		    taskpernode = int(m.group(2))
		    cores = int(m.group(3))
