...
namespace boost {
  ...
  public:
    ...
    // NGE: This is a nasty hack to forward an internal method of CSR
    // graphs that doesn't even exist for adjacency list or other graph types
    vertex_descriptor 
    make_vertex_descriptor(typename Graph::rank_type r, typename Graph::vertex_descriptor v)
    { return this->m_g.make_vertex_descriptor(r, v); }

    // NGE: Forward rank_type from base graph
    typedef typename Graph::rank_type rank_type;

    // NGE: Forward transport from base graph
    amplusplus::transport&       transport() const { return this->m_g.transport(); }
