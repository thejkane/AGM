...
#ifndef BOOST_GRAPH_TC_SPS
#define BOOST_GRAPH_TC_SPS
...
class triangle_counting_sps {

  typedef triangle_counting_sps<Graph, IdDistribution, NeighborMap, MessageGenerator> self_type;
...
public:
  ...
  void print_stats() {
    ...
    for (int i=0; i < nthreads; ++i) {
      ...
      std::cout << "Thread : " << i << " Max predecessors : " << max_pred_degree[i] << std::endl;
...
private:
  ...
  private:
    triangle_counting_sps* self;
  public:
    vertex_logicalid_comparator(triangle_counting_sps& s) : self(&s) {}
  ...
  std::vector<uint64_t> max_pred_degree;
#endif
...
#define TC_TYPE                                    \
      triangle_counting_sps<Graph, IdDistribution, NeighborMap, MessageGenerator>
...
TC_TYPE::initialize() {
  ...
  if (coalescing_size < pred_block_size) {
  ...
  max_pred_degree.resize(nthreads, 0);
#endif
...
template<TC_PARAMS>
template<typename IteSuc, typename ItePred>
void
TC_TYPE::many_set_intersection(IteSuc itesbegin,
			       uint64_t succount,
			       ItePred itepbegin,
			       uint64_t predcount,
			       int tid,
  ...
  IteSuc itesend = itesbegin + succount;
  ...
  std::vector<IteratorPair_t> predsuccs(predcount);
  for (int i=0; i < predcount; ++i) {
    Vertex s = *(itepbegin+i);
    predsuccs[i].first = std::lower_bound(vertex_successors[s].begin(),
					  vertex_successors[s].end(),
					  (*itesbegin));
    predsuccs[i].second = vertex_successors[s].end();
#ifdef TC_STATS
  ...
  for(int i=0; i < predsuccs.size(); ++i) {

    counting_output_iterator output_ite;

#ifdef TC_STATS
    tot_setsizes[tid] += (succount + (predsuccs[i].second - predsuccs[i].first));
    output_ite = std::set_intersection(itesbegin, itesend,
				       predsuccs[i].first,
				       predsuccs[i].second,
				       output_ite,
...
TC_TYPE::run(int tid) {
  AMPLUSPLUS_WITH_THREAD_ID(tid) {
    ...
    std::vector< SuccIterSizePair_t > rank_predecessors(transport.size());

    for (amplusplus::transport::rank_type r=0; 
	 r < transport.size(); ++r) {
      rank_predecessors[r].second = 0;
    }
	...
	auto numpredblocks = (pred_count+pred_block_size-1)/pred_block_size;
	auto numsucblocks = (succ_count+suc_block_size-1)/suc_block_size;
	
	//std::cout << "pred blocks : " << numpredblocks << std::endl;
	//std::cout << "suc blocks : " << numsucblocks << std::endl;
	
	for (auto pos = offset; pos < numsucblocks; pos=pos+nthreads) {
	  ...
	  if (max_pred_degree[tid] < pred_count)
	    max_pred_degree[tid] = pred_count;
#endif

	  auto begin = pos * suc_block_size;
	  size_t size = std::min(suc_block_size, (succ_count-(pos*suc_block_size)));
	  auto end = begin + size;

	  auto sucbegin = vertex_successors[*startite].begin() + begin;
	  auto sucend = vertex_successors[*startite].begin() + end;

	  for (uint64_t predblkpos = 0; predblkpos < numpredblocks; ++predblkpos) {
#ifdef PRINT_DEBUG
	    std::cout << "begin :" << begin << " end : " << end << std::endl;
#endif
	    auto predpos = predblkpos * pred_block_size;
	    auto predposend = predpos + std::min(pred_block_size, (pred_count-(predblkpos*pred_block_size)));
	    
	    for(; predpos < predposend; ++predpos) {

	      Vertex send_pred = *(vertex_predecessors[*startite].begin() + predpos);
	      amplusplus::transport::rank_type dest = get(owner, send_pred);	    
	      if (rank_predecessors[dest].second == 0) {
		rank_predecessors[dest].first = (vertex_predecessors[*startite].begin() + predpos);
	      }
	      
	      rank_predecessors[dest].second++;
	    }
	    ...
	    for (amplusplus::transport::rank_type r=0; 
		 r < transport.size(); ++r) {
	      if ((r != transport.rank()) && (rank_predecessors[r].second != 0)) {

#ifdef TC_STATS
		send_msgs[tid]++;
#endif

		block_msg bmsg(rank_predecessors[r].first, rank_predecessors[r].second,
			       sucbegin, size);
#ifdef TC_STATS
	    ...
	    if (rank_predecessors[transport.rank()].second != 0) {
#ifdef TC_STATS
	      local_msgs[tid]++;
	      auto sistart = std::chrono::system_clock::now();
#endif

	      many_set_intersection(sucbegin, size,
				    rank_predecessors[transport.rank()].first,
				    rank_predecessors[transport.rank()].second,
				    tid, true /*local call*/);
#ifdef TC_STATS
		...
		= std::chrono::duration_cast<std::chrono::nanoseconds>(siend - sistart).count();
	      ...
	      rank_predecessors[transport.rank()].second = 0;
	    }
	  }
	}
      }
...
processing_function {
  ...
  processing_function(triangle_counting_sps& self) : self(&self) {}
...
degree_processing_function {
  ...
  degree_processing_function(triangle_counting_sps& self) : self(&self) {}
