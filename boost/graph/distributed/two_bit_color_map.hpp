...
namespace amplusplus {
  template <> struct make_mpi_datatype<boost::two_bit_color_type> {MPI_Datatype get() const {return MPI_CHAR;}};
}

namespace boost {

template<typename GlobalMap, typename StorageMap>
class two_bit_color_map<local_property_map<GlobalMap,StorageMap> >
  : public parallel::distributed_property_map<GlobalMap,
                                              two_bit_color_map<StorageMap> >
{
  typedef two_bit_color_map<StorageMap> local_map;

  typedef parallel::distributed_property_map<GlobalMap, local_map >
    inherited;
...
get(two_bit_color_map<local_property_map<GlobalMap, StorageMap> > const& pm,
...
template<typename GlobalMap, typename StorageMap>
inline void
put(two_bit_color_map<local_property_map<GlobalMap, StorageMap> > const& pm, 
    typename two_bit_color_map<GlobalMap>::key_type key,
...
template<typename GlobalMap, typename StorageMap>
inline bool
exchange(two_bit_color_map<local_property_map<GlobalMap, StorageMap> > const& pm, 
	 typename two_bit_color_map<GlobalMap>::key_type key,
	 two_bit_color_type oldval, two_bit_color_type newval)
{
  return exchange(pm.base(), key, oldval, newval);
}

template<typename GlobalMap, typename StorageMap>
class two_bit_color_map<parallel::distributed_property_map<GlobalMap, StorageMap> > 
  : public parallel::distributed_property_map<
             GlobalMap, two_bit_color_map<StorageMap> >
{
  typedef two_bit_color_map<StorageMap> local_map;
...
get(two_bit_color_map<
      parallel::distributed_property_map<
        GlobalMap, two_bit_color_map<StorageMap> > > const& pm,
    typename two_bit_color_map<GlobalMap>::key_type key)
{
  return get(pm.base(), key);
}
