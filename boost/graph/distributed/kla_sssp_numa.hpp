...
#ifndef BOOST_GRAPH_KLA_SSSP_NUMA_HPP
#define BOOST_GRAPH_KLA_SSSP_NUMA_HPP
...
class kla_shortest_paths_numa {
  typedef kla_shortest_paths_numa<Graph, DistanceMap, EdgeWeightMap, PriorityQueueGenerator> 
    self_type;
...
public:
  ...
  kla_shortest_paths_numa(Graph& g,
...
#define KLA_SHORTEST_PATHS_NUMA_PARMS                                   \
      typename Graph, typename DistanceMap, typename EdgeWeightMap, typename WorkStats, typename Bucket, typename MessageGenerator

#define KLA_SHORTEST_PATHS_NUMA_TYPE                                    \
      kla_shortest_paths_numa<Graph, DistanceMap, EdgeWeightMap, WorkStats, Bucket, MessageGenerator>

template<KLA_SHORTEST_PATHS_NUMA_PARMS>
void
KLA_SHORTEST_PATHS_NUMA_TYPE::initialize()
{
...
template<KLA_SHORTEST_PATHS_NUMA_PARMS>
void
KLA_SHORTEST_PATHS_NUMA_TYPE::allocate_pqs() {
  for (BucketIndex i = 0 ; i < buckets.size() ; ++i) {
...
template<KLA_SHORTEST_PATHS_NUMA_PARMS>
void
KLA_SHORTEST_PATHS_NUMA_TYPE::initialize_threaded_buckets(int tid) {
  for (BucketIndex i = 0 ; i < buckets.size() ; ++i) {
...
template<KLA_SHORTEST_PATHS_NUMA_PARMS>
void
KLA_SHORTEST_PATHS_NUMA_TYPE::run(Vertex s, int tid)
{
...
template<KLA_SHORTEST_PATHS_NUMA_PARMS>
void
KLA_SHORTEST_PATHS_NUMA_TYPE::relax(const vertex_distance_data& data) 
{
...
template<KLA_SHORTEST_PATHS_NUMA_PARMS>
void
KLA_SHORTEST_PATHS_NUMA_TYPE::find_next_bucket()
{
...
template<KLA_SHORTEST_PATHS_NUMA_PARMS>
struct KLA_SHORTEST_PATHS_NUMA_TYPE::
vertex_distance_handler {
  
  vertex_distance_handler() : self(NULL) {}
  vertex_distance_handler(kla_shortest_paths_numa& self) : self(&self) {}
...
protected:
  kla_shortest_paths_numa* self;
};


} } } // end namespace boost::graph::distributed

#endif // BOOST_GRAPH_KLA_SHORTEST_PATHS_NUMA_HPP
