...
public:
  ...
  { 
    return get(owner, v) == g->transport().rank(); 
  }
...
public:
  ...
  amplusplus::transport&       transport() { return g.transport(); }       
  const amplusplus::transport& transport() const { return g.transport(); }
...
struct vertex_property_type<local_subgraph<Graph> > {
  typedef typename vertex_property_type<Graph>::type type;
};

template <typename Graph>
struct edge_property_type<local_subgraph<Graph> > {
  typedef typename edge_property_type<Graph>::type type;
};

template <typename Graph>
struct graph_property_type<local_subgraph<Graph> > {
  typedef typename graph_property_type<Graph>::type type;
};

template <typename Graph>
struct vertex_property_type<const local_subgraph<Graph> > {
  typedef typename vertex_property_type<Graph>::type type;
};

template <typename Graph>
struct edge_property_type<const local_subgraph<Graph> > {
  typedef typename edge_property_type<Graph>::type type;
};

template <typename Graph>
struct graph_property_type<const local_subgraph<Graph> > {
  typedef typename graph_property_type<Graph>::type type;
};
