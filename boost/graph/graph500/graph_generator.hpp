...
#define INITIATOR_A_NUMERATOR 5000
#define INITIATOR_BC_NUMERATOR 1000
#define INITIATOR_DENOMINATOR 10000
#elif EQ_GRAPH500_SPEC
...
static int generate_4way_bernoulli(mrg_state* st, int level, int nlevels,
...
#endif
  unsigned int adjusted_bc_numerator = rmatbc + spk_noise_factor;
  ...
  val -= adjusted_bc_numerator;
#if SPK_NOISE_LEVEL == 0
  if (val < rmata) return 0;
#else
  if (val < rmata * (INITIATOR_DENOMINATOR - 2 * rmatbc) / (INITIATOR_DENOMINATOR - 2 * adjusted_bc_numerator)) return 0;
#endif
  return 3;
}
...
static int generate_4way_bernoulli(mrg_state* st, int level, int nlevels) {
  return generate_4way_bernoulli(st, level, nlevels, INITIATOR_A_NUMERATOR,
...
void make_one_edge(int64_t nverts, int level, int lgN, mrg_state* st, packed_edge* result, uint64_t val0, uint64_t val1,
		   int32_t rmata, int32_t rmatbc) {
  int64_t base_src = 0, base_tgt = 0;
  while (nverts > 1) {
    int square = generate_4way_bernoulli(st, level, lgN, rmata, rmatbc);
...
static
void make_one_edge(int64_t nverts, int level, int lgN, mrg_state* st, packed_edge* result, uint64_t val0, uint64_t val1) {
  make_one_edge(nverts, level, lgN, st, result, val0, val1,
...
#pragma omp parallel for
  ...
  for (ei = start_edge; ei < end_edge; ++ei) {
    ...
    make_one_edge(nverts, 0, logN, &new_state, edges + (ei - start_edge), val0, val1, rmata, rmatbc);
  }
}
...
void generate_kronecker_range(
  ...
  generate_kronecker_range(seed, logN, start_edge, end_edge, edges, 
			  INITIATOR_A_NUMERATOR, INITIATOR_BC_NUMERATOR);
}
