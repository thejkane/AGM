...
template<typename PropertyMap, typename Key, typename Value>
inline bool
exchange(const caching_property_map<PropertyMap>& pm, const Key& key,
	 const Value& oldval, const Value& newval)
{ return exchange(pm.base(), key, oldval, newval); }

template<typename PropertyMap, typename Key, typename Value>
inline bool
maybe_exchange(const caching_property_map<PropertyMap>& pm, const Key& key,
	       const Value& oldval, const Value& newval)
{ return maybe_exchange(pm.base(), key, oldval, newval); }
...
// local_put is not defined for dummy_property_maps in property_map.hpp
