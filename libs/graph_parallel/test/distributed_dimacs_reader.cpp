...
void
test_dimacs_reader(amplusplus::transport& trans, const char *filename)
{
  ...
  boost::parallel::variant_distribution<> distrib = 
    boost::parallel::block<graph_traits<Graph>::vertices_size_type>(trans, reader.n_vertices());

  Graph g(dimacs_edge_iterator<dimacs_basic_reader>(reader), 
          dimacs_edge_iterator<dimacs_basic_reader>(end),
          reader.n_vertices(), trans, distrib);;
...
test_main(int argc, char* argv[])
{
  ...
  if (argc == 2) { 
    test_dimacs_reader(trans, argv[1]);
  }
