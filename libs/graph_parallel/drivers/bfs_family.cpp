...
#include "detail/graph_stats.hpp"
...
public:
  ...
  bool verify;
  int abort_level = std::numeric_limits<int>::max();
...
public:
  bool parse(int argc, char* argv[]) {
	...
	algorithms.push_back(algorithm);
      ...
      if (arg == "--abort-level") {
	int levels = boost::lexical_cast<int>( argv[i+1]);
	for (int p=0; p < levels; ++p) {
	  abort_levels.push_back(p);
	}
      }
  ...
  get_instance_params() {
	...
	BOOST_FOREACH(uint64_t s, sources) {
      ...
      if (sources.size() == 0) {
	BOOST_FOREACH(int alevel, abort_levels) {
	  BOOST_FOREACH(bfs_algorithm alg, algorithms) {
	    instance_params.push_back(bfs_instance_params(alg, verify, alevel));
	  }
	}
      } else {
	BOOST_FOREACH(uint64_t s, sources) {
	  BOOST_FOREACH(int alevel, abort_levels) {
	    BOOST_FOREACH(bfs_algorithm alg, algorithms) {
	      instance_params.push_back(bfs_instance_params(alg, verify, alevel, s));
	    }
	  }
	}
      }
...
private:
  ...
  run_bfs_chaotic(amplusplus::transport& trans, 
	...
	threads[i].swap(thr);
      ...
      D.set_abort_level(bfs_params.abort_level);
      D(0);
...
#ifdef COLLECT_STATS
    std::cout << "Level : " << bfs_params.abort_level << std::endl;
    D.print_stats();
#endif
...
int main(int argc, char* argv[]) {
  executor<BFSExecutor, bfs_params, bfs_instance_params> tc_executor;
